...
int main ()
{
    ...
    while (T--) {
        ...
        ll ans = 1;
        for (int i = 1; i <= n; ++i) {
            a[i] = (a[i-1] + 1ll*a[i]*pw[i-1]) % P;
            ans = ans * mpow(invpw[i-1], 2*(n-i+1)) % P;
        }


        vector<int> xx, yy, zz;
        int tmp = 1;
        for (int i = 1; i <= n; ++i) xx.pb(a[i]), yy.pb(0), tmp = 1ll * tmp * (-a[i]) % P;
        tmp = (tmp % P + P) % P;
        xx.pb(0); yy.pb(tmp);

        Poly g = tp.inter(xx, yy);



        for (int i = 0; i <= n; ++i) g.a[i] = 1ll*g.a[i]*(i+1)%P;



        for (int i = 0; i <= n; ++i) zz.pb(a[i]);
        vector<int> vs = tp.eval(g, zz);

        for (auto i: vs) ans = ans * i % P;
        ans = ans * ( ((1ll*n*(n+1)/2) & 1) ? -1 : 1);
        ans = (ans % P + P) % P;
